...
├── PLOTTING_MANIFEST.md        # Paper-figure script classification
└── tests/                      # Pytest suite
```
...
For the current JINST-oriented paper workflow, see `PLOTTING_MANIFEST.md` for the maintained split between main-paper required, supporting, and diagnostic/legacy plotting scripts. Diagnostic/legacy scripts are retained for checks and exploratory studies, but they are not required for the current minimum paper result.
...
## Paper and timing analysis workflow

The current manuscript is framed as a JINST-style detector/timing study. The main physics message is that high-resolution timing in the dual-readout calorimeter provides access to longitudinal shower development; the ML models are analysis tools used to quantify the PID impact of that detector information.

Timing-related inputs come from two data products:

- **Merged HDF5 files** (`h5s/*.h5py` or `/store/ml/dual-readout/h5s`): post-reconstruction event samples used by the training and plotting workflows. These contain reconstructed hit timing such as `DRcalo3dHits.time`, `DRcalo3dHits.time_end`, amplitudes, positions, `seed`, and `E_gen`.
- **Digi ROOT files** (`~/dream/tools/hdfs/<sample>/digi_*.root`): pre-merged digitization outputs used as the reference for MC truth timing and photon-count information. For timing validation, match HDF5 events to digi events with `seed` and `E_gen`.

Default timing-validation definitions:

- MC truth timing: amplitude-weighted mean of `RawCalorimeterHits/RawCalorimeterHits.timeStamp` from the matched digi event.
- Photon count: digi-derived and explicitly documented. Use raw amplitude sum for photon-statistics scaling studies; raw hit count may be used only as a labeled proxy.
- Reconstructed timing: derived from post-deconvolution HDF5 hit timing unless an explicit event-level reco timing key is provided.

The main timing validation figure is generated by:

```bash
micromamba run -n torch251 python plot_timing_reconstruction_validation.py --summary-json
```

The script produces vector-friendly PDF, high-resolution PNG, and JSON summary outputs under `plots/timing_reconstruction_validation/`. The figure is intended for the main manuscript and should show timing residuals, resolution versus photon count, and an example waveform before/after Wiener deconvolution.

The minimal additional timing-physics correlation figure is generated by:

```bash
micromamba run -n torch251 python plot_depth_vs_q50.py --summary-json
```

This writes `plots/depth_vs_q50/depth_vs_q50.pdf`, `.png`, and `.json`, showing the event-level correlation between longitudinal shower depth and reconstructed timing `q50`.

Use the `torch251` environment for plotting, HDF5 inspection, and ROOT/digi inspection. The system Python may have incompatible compiled packages such as `h5py`.
